...
select_cust <- function(data, keep=NULL, drop=NULL){
  stopifnot(is.null(keep) || is.null(drop))
  if(is.null(keep)){
    # col_nam_new <- colnames(data)
    col_nam_new <- colnames(data)[-match(drop,colnames(data))]
    return(data[col_nam_new])
  }
  if(is.null(drop)){
    # col_nam_new <- colnames(data)
    col_nam_new <- colnames(data)[match(keep,colnames(data))]
    return(data[col_nam_new])
  }
}
...
geom_ret <- function(prices, returns=FALSE){
  ...
  if(returns==FALSE){
    ...
    len <- length(prices)-1 #keep length(), since only weekdays in data
...
sharpe_ratio <- function(ret,
  ...
  expected_return <- ann_ret(prices = ret, days_ret=days_ret, returns = returns)
  
  # volatility returns
  vola_ret <- sqrt(var(ret))*days_vola
  
  # calculate risk free return
  risk_free <- ann_ret(prices = rf, days_ret=days_ret, returns=returns)
  
  # cut sharpe ratio at 0 for lack of interpretability for negative ratios
  sh_rat = max(0,(expected_return-risk_free)/vola_ret)
...
# customized performance measure
PI <- function(prices, rf, lambda, returns=FALSE){
  ...
  lambda <- lambda
  # convex combination
  PI <- lambda*ann_x_ret-(1-lambda)*ann_var_port
  return(PI)
}
...
#   !!! functions below are not required !!! 

...
verbose <- TRUE
loops <- FALSE # flag if you want to run the whole code or rely on previous results
```
...
Cross sectional standard deviation of average returns/SDs

$$
avg\_ret_i = \frac{1}{T_i}\sum_{t=1}^{T_i} (return_t-rf_t) \\ \\
$$
$$
SD\_ret_i = \sqrt{\frac{1}{T_i}\sum_{t=1}^{T_i}(return_t-avg\_ret_i)^2} \\ \\
$$
$$
...
```{r cross sectional SD, echo=FALSE}
...
if(loops==TRUE){
  ...
  saveRDS(lambda, file=paste0("lambda",".RDS"))
}else{
...
```{r main calculations, echo=FALSE}
...
if(loops==TRUE){
  ...
  # main loop through all available wiki IDs (takes +/- 10min)
  ignored_IDs <- 0
  for(i in 1:IDs){
  ...
  PIs <- readRDS("Customized_Performance_Measures.RDS")
...
plot(x = clean_PIs[,1], y=clean_SRs[,1], 
     xlab = "PIs",
     ylab = "SRs",
     main = paste0("Autocorrelation of PI and SR: ", round(corr_PI_SR,2)),
